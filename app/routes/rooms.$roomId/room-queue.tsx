...
export function RoomQueue() {
  ...
  return (
    <ul className="panel isolate border">
      {queue.items.map(({ id, submitter, track: { metadata } }) => (
        <li
          key={id}
          data-current={current?.id === id || undefined}
          className="-m-px flex flex-row gap-3 border border-transparent from-accent-200/10 p-3 data-[current]:border-accent-200/25 data-[current]:bg-gradient-to-r data-[current]:text-accent-200"
        >
          {metadata.artwork ? (
            <img
